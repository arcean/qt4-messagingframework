...
static const int MAX_INDEXER_RESTART_ATTEMPTS = 5;
static const char NO_WAIT_PARAM[] = "MAIL_BOOT_NO_WAIT";
      ...
      indexerRestartAttempt(0)
{
    ...
    QCopServer* server = new QCopServer(this);
    ...
    if (store->initializationState() != QMailStore::Initialized) {
        ...
        connect(server, SIGNAL(ready()), this, SLOT(startMessageIndexer()), Qt::QueuedConnection);
        connect(&msgIndexer, SIGNAL(finished(int, QProcess::ExitStatus)), this, SLOT(messageIndexerFinished(int, QProcess::ExitStatus)));
...
{
    msgIndexer.disconnect(this);
    if (msgIndexer.state() != QProcess::NotRunning)
        msgIndexer.terminate();
}
...
void MessageServer::startMessageIndexer()
{
    if (msgIndexer.state() != QProcess::NotRunning) {
        qWarning("Message indexer has been already started");
        return;
    }

    msgIndexer.start("msgindexer");
    if (!msgIndexer.waitForStarted())
    {
        qCritical("Unable to start message indexer");
    } else {
        qDebug("Message indexer started");
    }
}

void MessageServer::messageIndexerFinished(int code, QProcess::ExitStatus status)
{
    qWarning() << "Message indexer terminated, code =" << code << ", status =" << status << ", attempt =" << indexerRestartAttempt;

    if (QProcess::CrashExit == status) 
    {
        if (indexerRestartAttempt++ < MAX_INDEXER_RESTART_ATTEMPTS)
        {
            qputenv(NO_WAIT_PARAM, "1");
            startMessageIndexer();
        }
        else
        {
            qCritical("No more attempts to restart message indexer");
        }
    }
}
