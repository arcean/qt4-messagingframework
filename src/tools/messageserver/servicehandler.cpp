...
#include <qmailnamespace.h>
#include "../../libraries/qmfclient/support/qmailnamespace.h"
...
{
    return QMail::tempSystemPath() + "qmf-messageserver-requests";
}
...
const uint MaxSendFailedAttempts = 1;
...
{
    // HACK: if we have at least one enabled account in the list without registered
    // services, than we need to re-register accounts services.
    bool hasUnregisteredServices = checkUnregisteredServices(ids);
    // Only respond to updates that were generated by other processes
    if (QMailStore::instance()->asynchronousEmission() || hasUnregisteredServices) {
...
bool ServiceHandler::checkUnregisteredServices(const QMailAccountIdList &ids)
{
    foreach( const QMailAccountId& id, ids)
    {
        if (sourceMap.value(id) == NULL || sinkMap.value(id) == NULL)
        {
            return (QMailAccount(id).status() & QMailAccount::Enabled);
        }
    }
    return false;
}
...
void ServiceHandler::updateAction(quint64 action)
{
    ...
    if (it != mActionExpiry.end()) {
        ...
        if (mActiveActions.contains(action))
...
{
    emit actionStarted(qMakePair(action, MoveMessagesRequestType));
    QMailStore::instance()->flushIpcNotifications();
...
{
    emit actionStarted(qMakePair(action, FlagMessagesRequestType));
    QMailStore::instance()->flushIpcNotifications();
