...
QString PopClient::readResponse() 
{
    ...
    // if we are downloading a message for too long then to avoid
    // expire action time-out we are emiting progressChanged() signal
    if (lastStatusTimer.elapsed() > 60*1000) {
        lastStatusTimer.start();
        emit progressChanged(progressRetrievalSize, totalRetrievalSize);
    }
