...
{    
    if (!watcher) {
        ...
        ENFORCE (connect(watcher, SIGNAL(directoryChanged(QString)), this, SLOT(lockFileUpdated())));
        watcher->addPath(QMail::tempSystemPath());
    }
}

void QMailStoreImplementationBase::lockFileUpdated()
{
    if (!ipcChannel->isConnected() && QFile::exists(QMail::messageServerLockFilePath())) {
...
{
    if (QFile::exists(QMail::messageServerLockFilePath())) {
        ipcChannel->connectRepeatedly();
    } else {
        qDebug() << Q_FUNC_INFO << "messageserver is not running!";
        ipcConnectionFailed();
    }
}
