...
{
    QString path = QMail::tempSystemPath() + lockFile;
...
QString QMail::pluginsPath()
{
    ...
    // fall back to default plugins path
    return "/usr/lib/qmf/";
#endif
}
...
QString QMail::tempSystemPath()
{
    QDir subDir(QDir::tempPath() + QString("/messageserver"));
    if (!subDir.exists())
    {
        if (!subDir.mkpath(subDir.path()));
            qCritical() << "Cannot create <tmp>/messageserver directory";
    }
    return  subDir.path() + '/';
}
...
{
    static QString path(QMail::tempSystemPath() + QString("messageserver-instance.lock"));
    return path;
}
